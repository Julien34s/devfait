...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if Config.EnablePlayerManagement then 
			table.insert(elements, {label = _U('buy2', ESX.Math.GroupDigits(property.price)), value = 'buy2'})
		end 
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		elseif data.current.value == 'buy2' then 
			TriggerServerEvent('esx_property:buy2Property',test)
